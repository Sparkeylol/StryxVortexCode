                        ...
                        msg.author.send(embed).then((m) => {
                            m.channel.createMessageCollector(t => t.author.id === msg.author.id, {max: 1})
                                .on('collect', footer => {
                                    ...
                                    embed.setDescription("You can only enter a basic color. Please ensure to type it in all caps.\nExample: `GREEN`. Somethine like `LIGHT GREEN` may not work. If you embed doesn't send, please redo the command and pick a new color.")
                                    msg.author.send(embed).then((m) => {
                                        m.channel.createMessageCollector(t => t.author.id === msg.author.id, {max: 1})
                                            .on('collect', color => {
                                                ...
                                                embed.setFooter(`${sendEmbed.footer} | Powered by ${(guildSettings.branding ? "Vortex HQ" : client.user.username)}`)
