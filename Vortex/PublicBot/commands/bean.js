const Bean = {
    ...
    command: "bean",
    category: "Moderation",
    description: "Ban a user in style. :smirk:",
    ...
    execute: async function(client, msg, args, embed, guildSettings) {
        ...
        if (!args[0]) {
            embed.setTitle("Uh oh!")
            embed.setColor(15158332)
            embed.setDescription("Please provide a member you want me to bean!")
            return msg.channel.send(embed)
        }
        ...
        if (!reason) {
            embed.setTitle("Uh oh!")
            embed.setColor(15158332)
            embed.setDescription("Please provide a reason for the bean!")
            return msg.channel.send(embed)
        }
        ...
        embed.setTitle(`Beaned From ${msg.guild}`)
        embed.setDescription(`Greetings, ${target}, you have been beaned from ${msg.guild}.\n\n**Reason:** ${reason}\n**Moderator:** ${moderator}.\n`)
        embed.setColor(`RED`)

        await target.send(embed)


        target.ban({ reason: `Bean issued by: ${moderator.username}. Reason: ${reason}.`})
            .then((user) => {
                msg.channel.send({
                    embed: {
                        footer: {
                        ...
                        color: "GREEN",
                        title: "User Beaned!",
                        description: `Successfully beaned <@${user.id}>.\n**Reason:** ${reason}\n**Moderator:** <@${moderator.id}>.`,
                        timestamp: new Date(),

                    }
                })
                embed.setTitle("User Beaned")
                embed.setDescription(`**Member:** ${user}\n**Action Taken:** Bean/Ban\n**Reason:** ${reason}\n`)
                embed.setColor(`RED`)
                logchannel.send(embed)
            })
            .catch((err) => {
                console.log(err)
                msg.channel.send(`There was an error beaning <@${user.id}>.`)
                embeds.setTitle("Uh oh!")
                embed.setDescription(`There was an error beaning <@${iser.id}>.\n\n**Debugging:**\n||${err}||`)
            })    
            }
        }
}
