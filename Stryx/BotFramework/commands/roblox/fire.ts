...
class FireCommand extends Command {
  constructor() {
    super('fire', {
      aliases: ['fire'],
      description: {
        content: 'Fire a user in your Roblox group',
        ownerOnly: false,
  ...
  async exec(msg: Message, { robloxUsername } : { robloxUsername: string }) {
    ...
    let userId;
    try {
      userId = await roblox.getIdFromUsername(robloxUsername);
    } catch (err) {
      embed.setTitle('Uh oh!');
      embed.setDescription('There was an error firing this user.');
    ...
    try {
      await roblox.setRank(groupId, userId, 1);
    } catch (err) {
      embed.setTitle('Uh oh!');
