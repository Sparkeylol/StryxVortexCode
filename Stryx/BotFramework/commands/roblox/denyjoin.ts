...
class DenyJoinCommand extends Command {
  constructor() {
    super('denyjoin', {
      aliases: ['denyjoin', 'dj', 'deny'],
      description: {
        content: 'Deny a user\'s request to join your group.',
        ownerOnly: false,
        usage: '<roblox id>',
      },
      args: [
  ...
  async exec(msg: Message, { robloxId }: { robloxId: number }) {
    ...
    try {
      await roblox.handleJoinRequest(groupId, robloxId.toString(), false);
    } catch (err) {
      embed.setTitle('Uh oh!');
      embed.setDescription('There was an error denying this user.');
    ...
    await (this.client as StryxClient).logEvent({
      ...
      eventData: `<@${msg.author.id}> has denied **${username}**'s join request.`,
    });
...
module.exports = DenyJoinCommand;
