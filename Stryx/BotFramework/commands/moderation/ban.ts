...
class BanCommand extends Command {
  constructor() {
    super('ban', {
      aliases: ['ban', 'bean'],
      description: {
        content: 'Ban a user with an optional reason.',
        ownerOnly: false,
        usage: '<@user> [reason]',
      },
      args: [
  ...
  async exec(msg: Message, { user, reason }: { user: GuildMember, reason: string }) {
    ...
    if (!user) {
      const errorEmbed = new MessageEmbed();
      errorEmbed.setTitle('Uh oh!');
      errorEmbed.setDescription('You must provide a guild member to ban.');
      errorEmbed.setColor(guild?.settings.constants.colors.error as string);
      errorEmbed.setAuthor(msg.author.username, msg.author.avatarURL() as string);
      errorEmbed.setTimestamp();
    ...
    try {
      try {
        ...
        dmEmbed.setTitle(`Banned from ${msg.guild?.name}`);
        ...
        const errorEmbed = new MessageEmbed();
        errorEmbed.setTitle('Uh oh!');
        errorEmbed.setDescription('The user was banned, but their DMs are off, so I couldn\'t DM them.');
        errorEmbed.setColor(guild?.settings.constants.colors.warning as string);
        errorEmbed.setAuthor(msg.author.username, msg.author.avatarURL() as string);
        errorEmbed.setTimestamp();
        msg.channel.send(errorEmbed);
      } finally {
        await user.ban({ reason: `Ban issued by: ${msg.author.username}.${reason ? ` Reason: ${reason}` : ''}` });
      }
    } catch (err) {
      const errorEmbed = new MessageEmbed();
      errorEmbed.setTitle('Uh oh!');
      errorEmbed.setDescription('I couldn\'t ban the user!');
